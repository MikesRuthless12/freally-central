...
## 1.0.0 — the whole Freally family, embedded everywhere

Freally Central reaches 1.0 — the hub, and now the panel *inside* every Freally app.

- **View-only embedding mode**: the "Central inside" panel accepts an `allowDownloads={false}` prop that turns it into a pure showcase — the card grid, detail view, live release data, **real download counts**, and the **changelog viewer** all remain, while every Download / Install / Update / Open / Download-All control is hidden and an available app's card reads "Available" instead of "Download". Hosts that only surface the catalog can embed the panel with no download machinery at all — the Rust engine crate becomes optional, so downloads are impossible at the backend rather than merely hidden. No new user-facing strings; 18-locale parity stays green.
- **Brand rollout (FC-51)**: the panel is now embedded — view-only — in **Freally Sourcerer, File Manager, AV, and Studio**, each under a "More Freally apps" entry and localized through that host's own runtime. Those apps are built on Svelte, SolidJS, and React, so the React panel ships as a lazily code-split island where needed, resolving its `fcp-*` catalogs through each host's i18n. Freally Player & Vault (no UI yet) appear as catalog cards. Every brand app now surfaces the whole family.
- **Maintenance/hardening**: cargo-deny advisories/licenses clean (including the new `rustybuzz` unmaintained notice); vite patched; 0 npm vulnerabilities.
- **Housekeeping**: refreshed the Freally Vault card icon; retired Freally Snipper from the embedding docs and roadmap.
