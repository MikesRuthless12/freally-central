# Freally Central — UI strings (ja)
# Keys must stay in sync across all locales (npm run i18n:lint).

app-name = Freally Central
app-tagline = すべての Freally アプリのためのハブ。
filter-type = 種類
filter-all = すべて
available = 利用可能
coming-soon = 近日公開
install-all = すべてダウンロードしてインストール
install-all-hint = このコンピューター向けの利用可能なアプリをすべてダウンロードし、サイレントインストールします。
install-all-unsupported = インストールには Freally Central デスクトップアプリが必要です。
install-all-none = ダウンロード可能なアプリはまだありません。
language = 言語
theme-toggle = テーマを切り替え
card-view = 表示
detail-back = 戻る
detail-features = 機能
detail-started = 開始日 { $date }
detail-visit-site = サイトを見る
detail-no-features = 機能の詳細は近日公開。
detail-coming-soon-note = このアプリはまだダウンロードできません。
status-loading = カタログを読み込み中…
status-offline = オフライン — 内蔵カタログを表示中。
grid-empty = このフィルターに一致するアプリはありません。
eula-title = エンドユーザー使用許諾契約
eula-intro = 続行するにはライセンスを読んで同意してください。
eula-accept = 同意して続行
eula-decline = 拒否
eula-scroll-hint = 最後までスクロールすると同意が有効になります。
footer-note = ローカル優先 · アカウント不要 · テレメトリなし
...
about-first-stable = 最初の安定版
about-first-stable-pending = 未リリース
about-platform = プラットフォーム
about-local-first = ローカル優先で誠実 — アカウント不要、テレメトリなし。
...
updates-check-again = 再確認
updates-available = バージョン { $version } が利用可能です。
updates-current-version = (現在 { $current })
updates-release-notes-label = { $version } のリリースノート
updates-confirm = 今すぐダウンロードしてインストールしますか？
updates-yes-update-now = 今すぐ更新
updates-no-not-now = 後で
updates-downloading = { $version } をダウンロード中
updates-starting = 開始中…
...
status-update = アップデートあり
status-not-installed = 未インストール
action-install = インストール
action-update = アップデート
action-redownload = 再ダウンロード

# --- Downloads & live progress (Phase 4) ---
dl-downloading = ダウンロード中…
dl-verifying = 検証中…
dl-done-verified = ダウンロード済み・検証済み
dl-done-size-only = ダウンロード済み（サイズ確認済み）
dl-failed = ダウンロードに失敗しました。
dl-failed-network = ダウンロードに失敗しました — ネットワークエラー。
dl-failed-verify = 検証に失敗しました — ファイルは破棄されました。
dl-failed-busy = このアプリのダウンロードはすでに実行中です。
dl-canceled = ダウンロードをキャンセルしました。
dl-cancel = キャンセル
dl-retry = 再試行
dl-progress-label = { $name } のダウンロード進行状況
dl-show-in-folder = フォルダーで表示
batch-progress = { $total } 件中 { $done } 件ダウンロード済み
batch-cancel-all = すべてキャンセル
batch-done = すべてのダウンロードが完了しました。
batch-failed = { $total } 件中 { $failed } 件のダウンロードが失敗しました。
batch-canceled = { $total } 件中 { $canceled } 件のダウンロードをキャンセルしました。

# --- Silent install (Phase 5) ---
dl-installing = インストール中…
install-progress-label = { $name } のインストール進行状況
install-done = インストール済み ✓
install-canceled = インストールをキャンセルしました。
install-failed = インストールに失敗しました。
install-failed-not-downloaded = インストールの前にアプリをダウンロードしてください。
install-failed-not-verified = このインストーラーはサイレントインストール用に検証されていないため、実行されませんでした。
install-failed-verify = 検証に失敗したため、インストーラーを拒否しました。
install-failed-unsupported = この種類のインストーラーは、このコンピューターではサイレントインストールできません。
install-failed-installer = インストーラーがエラーを報告しました。
install-failed-elevation = 管理者権限が拒否されました。
install-failed-busy = すでにインストールが実行中です。
open-app = 開く
open-failed = { $name } を開けませんでした。
batch-installing = インストール中… { $total } 件中 { $done } 件インストール済み
batch-installed = すべてのアプリをインストールしました。
batch-install-failed = { $total } 件中 { $failed } 件のインストールが失敗しました。
batch-install-canceled = { $total } 件中 { $canceled } 件のインストールをキャンセルしました。
