...
install-all = Alles downloaden & installeren
install-all-hint = Downloadt en installeert stil elke beschikbare app voor deze computer.
install-all-unsupported = Installeren vereist de Freally Central-desktop-app.
install-all-none = Nog geen downloadbare apps.
language = Taal
...
dl-installing = Installeren…
install-progress-label = Installatievoortgang voor { $name }
install-done = Geïnstalleerd ✓
install-canceled = Installatie geannuleerd.
install-failed = Installatie mislukt.
install-failed-not-downloaded = Download de app voordat je installeert.
install-failed-not-verified = Dit installatieprogramma is niet geverifieerd voor stille installatie — het is niet uitgevoerd.
install-failed-verify = Verificatie mislukt — het installatieprogramma is geweigerd.
install-failed-unsupported = Dit type installatieprogramma kan niet stil worden geïnstalleerd op deze computer.
install-failed-installer = Het installatieprogramma meldde een fout.
install-failed-elevation = Beheerderstoestemming is geweigerd.
install-failed-busy = Er loopt al een installatie.
open-app = Openen
open-failed = Kan { $name } niet openen.
batch-installing = Installeren… { $done } van { $total } geïnstalleerd
batch-installed = Alle apps geïnstalleerd.
batch-install-failed = { $failed } van { $total } installaties mislukt.
batch-install-canceled = { $canceled } van { $total } installaties geannuleerd.
