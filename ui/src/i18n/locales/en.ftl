...
install-all = Download & install all
install-all-hint = Download and silently install every available app for this computer.
install-all-unsupported = Installing needs the Freally Central desktop app.
install-all-none = No downloadable apps yet.
language = Language
...
install-progress-label = Install progress for { $name }
install-done = Installed ✓
install-canceled = Install canceled.
install-failed = Install failed.
install-failed-not-downloaded = Download the app before installing.
install-failed-not-verified = This installer isn’t verified for silent install — it was not run.
install-failed-verify = Verification failed — the installer was refused.
install-failed-unsupported = This installer type can’t be installed silently on this computer.
install-failed-installer = The installer reported an error.
install-failed-elevation = Administrator permission was declined.
install-failed-busy = An install is already running.
open-app = Open
open-failed = Couldn’t open { $name }.
batch-installing = Installing… { $done } of { $total } installed
batch-installed = All apps installed.
batch-install-failed = { $failed } of { $total } installs failed.
batch-install-canceled = { $canceled } of { $total } installs canceled.
