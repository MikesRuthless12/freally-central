...
const tauriEngine: DownloadEngine = {
  async start(request: DownloadRequest, onEvent: (event: DownloadEvent) => void): Promise<void> {
    ...
    await invoke("start_download", { request, onEvent: channel });
  },
...
/**
 * This machine's OS + arch, for installer selection — from the backend's build
 * info inside the shell, or null in a plain browser (downloads are off there).
 */
