...
import type { OsKey, ReleaseInfo, ReleaseState } from "./types";
...
const OS_KEYS: OsKey[] = ["windows", "macos", "linux"];
...
// Total downloads of every asset whose name matches an OS's installer regex.
// Summed (not first-match) so a platform shipping several installers — Linux
// .AppImage + .deb + .rpm — reports its full figure. Returns undefined when the
// pattern is absent or matches nothing, so callers can omit that OS entirely.
// Invalid patterns are ignored (match nothing) rather than throwing.
function sumMatching(assets: RawAsset[], pattern: string | undefined): number | undefined {
  ...
  for (const asset of assets) {
    ...
    if (name && re.test(name)) {
      sum += num(asset.download_count);
      matched = true;
    }
  }
  return matched ? sum : undefined;
}
...
export function normalizeRelease(raw: RawRelease, assets: AppAssets | undefined): ReleaseInfo {
  ...
  for (const os of OS_KEYS) {
    const count = sumMatching(rawAssets, assets?.[os]);
    if (count !== undefined) {
      perOs[os] = count;
